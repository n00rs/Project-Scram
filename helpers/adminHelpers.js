      ...
      db.get()
        ...
        .then((result) => {
          ...
          if (result[0] == null) {
            db.get()
              .collection(collection.COUPONCOLLECTION)
              .createIndex({ couponName: 1 }, { unique: true });
    ...
    return new Promise((resolve, reject) => {
      db.get()
        ...
        .then((product) => {
          ...
          let checkStock = product.modelDetails.size.find((arr) =>
            arr.size === selectedSize && +arr.stock >= quantity ? true : resolve(stockOutMsg)
          );
          ...
          if (checkStock) {
            ...
            db.get()
              .collection(collection.PRODUCTCOLLECTION)
              .updateOne(
                { _id: objectId(prodId), "modelDetails.size.size": selectedSize },
                {
              ...
              .then((result) => {
                if (result.modifiedCount == 1) {
                  db.get()
                    .collection(collection.ORDERCOLLECTION)
                    .updateOne(
                      { _id: orderId, "orderData.items.item": objectId(prodId) },
                      {
              ...
              .catch((e) => {
                reject({ error: "mongo loose mind2" });
              });
          }
        })
