...
$("#updateName").validate({
  ...
  submitHandler: function (form) {
    $.ajax({
      ...
      method: "PUT",
      success: (result) => {
        if (result.status) $("#name").load(location.href + " #name");
        else swal({ title: "oops somthing went wrong" });
      },
    });
  },
});
...
jQuery.validator.addMethod(
  ...
  function (phone_number, element) {
    return (
      ...
      (phone_number.length >= 13 && phone_number.match(/^((\+91[0-9]{10}))$/))
    );
  },
  ...
  submitHandler: (form) => {
    ...
    $.ajax({
      ...
      success: (res) => {
        ...
        } else swal(`oops google is out of service let me look into it `);
      },
    });
  },
});
